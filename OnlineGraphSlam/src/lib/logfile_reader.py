...
class LegoLogfile(object):
        ...
        for l in f:
            ...
            #   define the center point of the ellipse.
                ...
                if sp[1] == 'C':
                    ...
                    cyl = list(map(float, sp[2:]))
                    self.world_cylinders.append([(cyl[2*i], cyl[2*i+1]) for i in range( int(len(cyl)/2)) ])
                elif sp[1] == 'E':
                    if first_world_ellipses:
                        self.world_ellipses = []
                        first_world_ellipses = False
                    ell = list(map(float, sp[2:]))
    ...
    def scanner_to_world(pose, point):
        """Given a robot pose (rx, ry, heading) wrong -> scanner pose ?
        and a point (x, y) in the scanner's coordinate system,
        return the point's coordinates in the world coordinate system."""
        dx = cos(pose[2])
        dy = sin(pose[2])
        x, y = point
    ...
    #LK- Implementation
    @staticmethod
    def get_scanner_pose(vehicle, scanner_offset):
        """Given vehicle pose (x,y,w) and the scanner offset (dxs, dys, dws)
        return the scanner pose (xs,ys,ws) in world coordinate"""
        dxs = scanner_offset[0]
        dys = scanner_offset[1]
        dws = scanner_offset[2]
        x  = vehicle[0]
        y  = vehicle[1]
        w   = vehicle[2]
        xs  = dxs*cos(w) - dys*sin(w) + x
        ys  = dxs*sin(w) + dys*cos(w) + y
        ws  = (w + dws + pi) % (2*pi) - pi 
        return (xs, ys, ws)
