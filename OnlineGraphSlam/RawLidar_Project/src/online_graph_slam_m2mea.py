...
class OnlineGraphSLAM:
    ...
    def motion_update(self, control):
        ...
        # get motion command dx,dy,dtheta
        motion = self.dg(self.state[0:3], control, self.robot_width)
        ...
        for b in range(6):
            # set motion noise 
            if (b == 2 or b == 5):
                motion_noise = motion_noise_theta
            else:
        ...
        for b in range(3):
            ...
            self.Omega.value[b  ][b+3] += -1.0 / motion_noise
            self.Omega.value[b+3][b  ] += -1.0 / motion_noise
            # update vector e.g. (x0,x1),(y0,y1)
            self.Xi.value[b  ][0]  += -motion[b] / motion_noise
            self.Xi.value[b+3][0]  +=  motion[b] / motion_noise
    ...
    def measurement_update(self, observations):
        ...
        for obs in observations:
            # extract data
            range_bearing, cylinder_world, cylinder_scanner, cylinder_index = obs
            
            lmk_index = cylinder_index
            dx_lmk = cylinder_world[0]-self.state[0]
            dy_lmk = cylinder_world[1]-self.state[1]
            ...
            for b in range(2):
                self.Omega.value[b+3][b+3]     +=  1.0 / self.measurement_noise
                self.Omega.value[m+b+3][m+b+3] +=  1.0 / self.measurement_noise
                self.Omega.value[b+3][m+b+3]   += -1.0 / self.measurement_noise
                self.Omega.value[m+b+3][b+3]   += -1.0 / self.measurement_noise
                self.Xi.value[b+3][0]          += -measurement[1+b] / self.measurement_noise
                self.Xi.value[m+b+3][0]        +=  measurement[1+b] / self.measurement_noise
                
                
                
    def compute_onlineslam(self):
...
if __name__ == '__main__':
    ...
    # Filter constants.
    motion_noise_xy = 1.0                   # Weight error in x,y motor control
    motion_noise_theta = 1.0                # Weight error in steering control 
    measurement_noise = 0.2                 # Weight error in x,y measurement
    ...
    for i in range(len(logfile.motor_ticks)):
        ...
        xs = gph.state[0] + scanner_displacement * cos(gph.state[2])
        ys = gph.state[1] + scanner_displacement * sin(gph.state[2])
        ts = gph.state[2]
